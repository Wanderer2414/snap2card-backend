...
function safeGenerationLog(request: VocabularyFromTextRequest, outputCount: number, latencyMs: number): void {
  console.log(
    ...
    JSON.stringify({ sourceType: request.sourceType, inputCharacters: request.text.length, requestedCount: request.count, outputCount, latencyMs })
  );
}
...
function safeGenerationErrorLog(request: VocabularyFromTextRequest, error: unknown): void {
  ...
  console.log(
    ...
    JSON.stringify({ sourceType: request.sourceType, inputCharacters: request.text.length, requestedCount: request.count, category })
  );
}
